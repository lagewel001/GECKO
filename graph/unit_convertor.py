...
from graph._QUDT_UNIT import QUDT_UNIT
...
def _generate_defined_namespace():
    g = Graph()
    g.parse('qudt_unit.ttl', format='n3')

    with open(f"_QUDT_UNIT.py", 'w', encoding='UTF-8') as f:
        f.write(inspect.cleandoc(f"""
            ...
            class QUDT_UNIT(DefinedNamespace):
                \"""
                {g.value(g.value(None, RDF.type, OWL.Ontology), RDFS.label)}
        ...
        for subject, predicate, object in g.triples((None, RDF.type, URIRef('http://qudt.org/schema/qudt/Unit'))):
            assert isinstance(predicate, URIRef)
            f.write(
                f"    {subject.split('/')[-1].replace('-', '_')} = URIRef(\"{subject}\")  # {' '.join((g.value(subject, DCT.description) or '').splitlines())}\n")

        f.write(f"\n    _NS = Namespace(\"{g.value(None, RDF.type, OWL.Ontology)}\")\n")
