...
from pipeline.sparql_controller import get_table_graph
...
class SExpression:
    ...
    def get_admissible_tokens(self) -> List[str]:
        ...
        for T in candidates:
            ...
            match T.__name__:
                ...
                case Code.__name__:
                    # Get all possible measures or dims corresponding with table as admissible tokens
            ...
            case _:
                ...
                if type_ is None:
                    ...
                    if isinstance(self._current_node, AGGREGATION):
                        ...
                        self.obs_map[self._current_table].measures.add(node)
    ...
    def get_valid_msr_tokens(self) -> Set[str]:
        """Return all valid measures of table corresponding with the current expression"""
        measures = set(self.graph.objects(self._current_table.uri, QB.measure))

        # Exclude measures already present in expression
        return ({uri_to_code(msr) for msr in measures} -
                set(str(m) for m in self.obs_map[self._current_table].measures))
    ...
    @staticmethod
    def parse(sexp: str, graph: Optional[Graph] = None) -> 'SExpression':
        """
            Parse a string into a valid S-expresiion.
        ...
        for termtypes in re.finditer(TERM_REGEX, sexp):
            ...
            if isinstance(node, TABLE):
                if expression.graph is None or len(expression.graph) == 0:
                    expression.graph = get_table_graph(node)
                if expression.graph is None or len(expression.graph) == 0:
